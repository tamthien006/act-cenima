...
// Doanh thu theo thời gian + tổng quan
exports.getRevenueStats = async (req, res, next) => {
  try {
    ...
    // For Ticket revenue: accept confirmedAt in range OR (no confirmedAt and createdAt in range)
    const ticketMatch = {
      status: 'confirmed',
      $or: [
        {
        ...
        {
          $or: [{ confirmedAt: null }, { confirmedAt: { $exists: false } }],
          createdAt: {
            $gte: startOfDay(new Date(startDate)),
            $lte: endOfDay(new Date(endDate))
          }
    ...
    switch (groupBy) {
      case 'hour':
        ...
        break;
      case 'week':
        // vẫn nhóm theo năm + tuần
        dateFormat = '%Y-W%U';
        groupByQuery = {
          year: { $year: '$confirmedAt' },
          week: { $week: '$confirmedAt' }
        };
        break;
      case 'month':
        dateFormat = '%Y-%m';
    ...
    // Compute usedDate = confirmedAt || createdAt, usedAmount = totalPrice || totalAmount
    const revenueByPeriod = await Ticket.aggregate([
      { $match: ticketMatch },
      { $addFields: { usedDate: { $ifNull: ['$confirmedAt', '$createdAt'] }, usedAmount: { $ifNull: ['$totalPrice', '$totalAmount'] } } },
      {
        $group: {
          _id: {
            year: { $year: '$usedDate' },
            ...
            hour: groupBy === 'hour' ? { $hour: '$usedDate' } : undefined
          },
          date: { $first: '$usedDate' },
          totalAmount: { $sum: '$usedAmount' },
          ticketCount: { $sum: 1 },
          averageTicketValue: { $avg: '$usedAmount' }
        }
      },
    ...
    const summaryAgg = await Ticket.aggregate([
      { $match: ticketMatch },
      { $addFields: { usedAmount: { $ifNull: ['$totalPrice', '$totalAmount'] } } },
      {
        $group: {
          _id: null,
          totalRevenue: { $sum: '$usedAmount' },
          totalTickets: { $sum: 1 },
          averageOrderValue: { $avg: '$usedAmount' },
          maxSingleOrder: { $max: '$usedAmount' }
        }
      }
    ]);
    ...
    // Top phim theo doanh thu từ ticket đã confirmed
...
exports.getTopMovies = async (req, res, next) => {
  try {
    ...
    const topMovies = await Ticket.aggregate([
      { $match: matchQuery },
      { $group: { _id: '$movieId', ticketCount: { $sum: 1 }, totalRevenue: { $sum: '$totalPrice' } } },
      { $sort: { [sortField]: sortOrder } },
      { $limit: parseInt(limit, 10) },
      { $lookup: { from: 'movies', localField: '_id', foreignField: '_id', as: 'movie' } },
      { $unwind: '$movie' },
      { $project: { _id: 0, movieId: '$_id', title: '$movie.title', posterUrl: '$movie.posterUrl', ticketCount: 1, totalRevenue: 1 } }
    ]);

    const summary = await Ticket.aggregate([
      { $match: matchQuery },
      { $group: { _id: null, totalTickets: { $sum: 1 }, totalRevenue: { $sum: '$totalPrice' }, avgTicketPrice: { $avg: '$totalPrice' } } },
      { $project: { _id: 0, totalTickets: 1, totalRevenue: 1, avgTicketPrice: { $round: ['$avgTicketPrice', 2] } } }
    ]);
    ...
    res.status(200).json({
      ...
      data: {
        movies: topMovies,
        summary: summary[0] || { totalTickets: 0, totalRevenue: 0, avgTicketPrice: 0 }
      },
      meta: { sortedBy: by, startDate, endDate, limit: parseInt(limit, 10) }
    });
  } catch (err) {
    next(err);
  }
...
exports.getCinemaPerformance = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: { cinemas: cinemaPerformance, summary },
...
exports.getUserActivity = async (req, res, next) => {
  try {
    ...
    const userActivity = await Movie.db.model('User').aggregate([
      {
        $match: {
          createdAt: {
            $gte: startOfDay(new Date(startDate)),
    ...
    const engagementStats = await Ticket.aggregate([
      ...
      },
      {
        $group: {
          ...
          repeatCustomers: { $sum: { $cond: [{ $gt: ['$ticketCount', 1] }, 1, 0] } },
          newCustomers: { $sum: { $cond: [{ $eq: ['$ticketCount', 1] }, 1, 0] } }
        }
      },
      {
        $project: {
          ...
          newCustomers: 1,
          repeatCustomerRate: {
            $multiply: [{ $divide: ['$repeatCustomers', { $max: [1, '$totalUsers'] }] }, 100]
          }
        }
      }
    ]);
    ...
    const topUsers = await Ticket.aggregate([
      ...
      { $unwind: '$user' },
      {
        $project: {
          ...
          totalSpent: 1,
          lastPurchase: 1,
          averageTicketValue: { $divide: ['$totalSpent', '$ticketCount'] }
        }
      }
    ]);
